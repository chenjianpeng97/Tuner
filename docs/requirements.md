...
- Part A:动态在线表格平台能力（独立组件）
  - 目标:提供通用在线表格能力，不绑定特定业务语义
  - rule:对外接口以表格原子操作为中心（表、视图、行、列、单元格、批处理、资产绑定）
- Part B:需求管理模板能力（业务模板）
  - 目标:基于 Part A 的表格能力，预制需求管理模板
  - rule:项目跟踪视图与产品需求列表均以“模板”形式存在，不作为底层平台固定模型

## Part A-1 表格与模板实例管理（动态在线表格）

- 创建模板实例表:系统支持基于模板创建表格实例
  - rule:表格实例归属项目
- 维护表结构:支持列增删改、列顺序调整
  - rule:支持批量结构操作
- 维护表数据:支持行增删改、单元格批量写入
  - rule:支持批量数据操作
- 维护视图:支持一个表对应多个视图（如 grid、kanban、gantt）
  - rule:视图可配置筛选、排序、隐藏列
- 并发与幂等:支持版本控制与幂等写入
  - rule:写操作支持版本校验（expected_version）
  - rule:批量和上传类操作支持幂等键

## Part A-2 资产挂接与通用统计（动态在线表格）

- 资产上传:支持在项目上下文上传资产
  - rule:资产必须归属 project_id
- 资产绑定:支持将资产绑定到表格单元格或行
  - rule:支持先上传后绑定
- 资产复制与跟随:支持资产副本与父级跟随关系
  - rule:副本可脱离形成独立分支
- 通用统计:支持表级统计与活动日志
  - rule:支持无关联资产独立统计

## Part B-1 项目跟踪视图模板（需求管理业务模板）
...
## Part B-4 事项与需求一体化模板规则（需求管理业务模板）
